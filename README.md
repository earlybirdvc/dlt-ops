...
The package never caps dlt: the dependency is a floor (`dlt>=1.27`) and you own your project's dlt version. Nothing gates on a list of dlt minors — every feature runs on any dlt at or above the floor, including a release newer than the tested matrix. That matrix records what CI exercised per dlt minor × destination and lives in [COMPATIBILITY.md](COMPATIBILITY.md).
