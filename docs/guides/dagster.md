---
description: Schedule a dlt-ops project on Dagster — a CLI-driven asset per source from discover_sources() scheduled with AutomationCondition.on_cron that keeps the dlt-ops ledger, assertions, checkpoints, and backfill, plus the advanced native @dlt_assets recipe for per-table lineage and the operational trade-off it makes.
---
...
| Recipe | Unit in Dagster | Keeps dlt-ops ledger, assertions, checkpoints, backfill | Per-table lineage | API surface it needs |
|---|---|---|---|---|
...
- **Dagster owns the run, so the dlt-ops operational layer is bypassed.** `@dlt_assets` calls `dlt.run` itself; it never goes through `dlt-ops pipeline run`. That means no runs-ledger row, no pre-load `fail`/`warn` assertions, no checkpoint resume, and no backfill window injection — everything that lives in the dlt-ops runner. You gain per-table lineage and give up the operational guarantees the CLI-driven asset keeps.
