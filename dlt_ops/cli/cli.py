...
@click.group()
# Reads the installed distribution rather than dlt_ops.__version__, so a package
# running from a stale install reports what is actually on the path — the number
# a bug report needs.
@click.version_option(package_name="dlt-ops")
@click.option(
    "--root",
    "-r",
    "project_root",
